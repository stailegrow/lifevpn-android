...
import com.stailegrow.maxstrike.ui.components.LaunchSplash
        ...
        setContent {
            MaxStrikeTheme {
                Surface(
                    ...
                    // Экран запуска показывается один раз за жизнь Activity —
                    // не на каждую рекомпозицию и не после смены темы/языка,
                    // поэтому remember без ключей.
                    var showSplash by remember { mutableStateOf(true) }
                    if (showSplash) {
                        LaunchSplash(onFinished = { showSplash = false })
                    } else {
