 ...
 */
val LocalPalette = staticCompositionLocalOf { Palette.sky }

/**
 * Семь пресетов (5 светлых + Ночь + AMOLED) вместо одной HUD-схемы. ThemeStore хранит
...
fun MaxStrikeTheme(content: @Composable () -> Unit) {
    ...
    // Function-reference на перегруженную darkColorScheme/lightColorScheme
    // неоднозначен для компилятора при вызове с именованными аргументами,
    // поэтому просто дублируем вызов в двух ветках, а не пытаемся выбрать
    // "конструктор" единым выражением.
